(() => {
  ...
  const UNIT_BAG_ASSET_ID = 93889631653195;
  const UNIT_BAG_THUMBNAIL = `https://www.roblox.com/asset-thumbnail/image?assetId=${UNIT_BAG_ASSET_ID}&width=150&height=150&format=png`;
  ...
  const UNIT_IMAGES = {
    ...
    // Legendary
    'forbidden teacher': '/assets/units/forbidden-teacher.png',
    'greed': '/assets/units/greed.png',
    'ice queen': '/assets/units/ice-queen.png',
    'ramen guy': '/assets/units/ramen-guy.png',
    'scissor': '/assets/units/scissor.png',
    'reaper': '/assets/units/reaper.png',
    'the hero': '/assets/units/the-hero.png',
    'water princess': '/assets/units/water-princess.png',

    // Epic
    'bounty hunter': '/assets/units/bounty-hunter.png',
    'corps captain': '/assets/units/corps-captain.png',
    'demon cyborg': '/assets/units/demon-cyborg.png',
    'ice mage': '/assets/units/ice-mage.png',
    'nen hunter': '/assets/units/nen-hunter.png',
    'stone alchemist': '/assets/units/stone-alchemist.png',

    // Rare
    'curly brow': '/assets/units/curly-brow.png',
    'kid assassin': '/assets/units/kid-assassin.png',
    'reishi archer': '/assets/units/reishi-archer.png',
    'rubber boy': '/assets/units/rubber-boy.png',
    'carrot': '/assets/units/carrot.png',
    'thunder shinobi': '/assets/units/thunder-shinobi.png',
  };
  ...
  const elements = {
    ...
    inventoryResultCount: $('inventory-result-count'),
    inventoryLoadMore: $('inventory-load-more'),
    ...
    unitExclusiveCount: $('unit-exclusive-count'),
    unitLegendaryCount: $('unit-legendary-count'),
    unitEpicCount: $('unit-epic-count'),
    unitRareCount: $('unit-rare-count'),
    unitSearch: $('unit-search'),
    unitRarity: $('unit-rarity'),
    unitTrait: $('unit-trait'),
    unitGrid: $('unit-grid'),
    unitEmpty: $('unit-empty'),
    unitResultCount: $('unit-result-count'),
    unitLoadMore: $('unit-load-more'),
  ...
  function clockTime(dateValue) {
    ...
    return date.toLocaleTimeString('th-TH', { timeZone: THAI_TIME_ZONE, hour: '2-digit', minute: '2-digit', hour12: false });
  }
  ...
  function unitBagIconHtml(className = 'units-button-icon') {
    const imageUrl = assetIconUrl(UNIT_BAG_ASSET_ID) || UNIT_BAG_THUMBNAIL;
    return `<img class="${escapeHtml(className)}" src="${escapeHtml(imageUrl)}" alt="Unit Vault" loading="eager" decoding="async">`;
  }
        ...
        <div class="card-action">
          <div class="card-action-buttons">
            <button class="inventory-button" type="button" data-inventory-user-id="${escapeHtml(player.userId)}">
              <span class="action-icon-shell">${assetIconHtml(BAG_ASSET_ID, 'Inventory', 'inventory-button-icon', '▣')}</span><span>View Inventory</span>
            </button>
            <button class="units-button" type="button" data-unit-user-id="${escapeHtml(player.userId)}">
              <span class="action-icon-shell">${unitBagIconHtml()}</span><span>View Units</span>
            </button>
  ...
  function renderInventoryModal() {
    ...
    state.inventoryVersion = String(player.updatedAt || '');
    ...
    const visibleItems = items.slice(0, state.inventoryLimit);
    elements.itemGrid.innerHTML = visibleItems.map((item) => `
    ...
    elements.inventoryResultCount.textContent = `${formatNumber(visibleItems.length)} / ${formatNumber(items.length)} items`;
    elements.inventoryLoadMore.hidden = remaining === 0;
    elements.inventoryLoadMore.textContent = remaining > 0 ? `Load more (${formatNumber(remaining)})` : '';
  ...
  function renderUnitModal() {
    ...
    state.unitVersion = String(player.updatedAt || '');
    ...
    const rarityCount = (rarity) => allUnits.filter((unit) => String(unit.rarity || '').toLowerCase() === rarity).length;

    elements.unitCount.textContent = formatNumber(allUnits.length);
    elements.unitSecretCount.textContent = formatNumber(rarityCount('secret'));
    elements.unitExclusiveCount.textContent = formatNumber(rarityCount('exclusive'));
    elements.unitMythicCount.textContent = formatNumber(rarityCount('mythic'));
    elements.unitLegendaryCount.textContent = formatNumber(rarityCount('legendary'));
    elements.unitEpicCount.textContent = formatNumber(rarityCount('epic'));
    elements.unitRareCount.textContent = formatNumber(rarityCount('rare'));
    ...
    const units = allUnits.filter((unit) => {
      const matchesQuery = !query || [unit.name, unit.asset, unit.trait, unit.rarity]
    ...
    const visibleUnits = units.slice(0, state.unitLimit);
    elements.unitGrid.innerHTML = visibleUnits.map((unit) => {
      const image = unitImage(unit);
      const rarity = unit.rarity || 'Unknown';
      const name = unit.name || unit.asset || 'Unknown Unit';
      return `
        <article class="unit-card rarity-${rarityClass(rarity)}">
          <div class="unit-image-wrap">
            ${image
              ...
              : `<span class="unit-image-fallback">${escapeHtml(String(name).slice(0, 2).toUpperCase())}</span>`}
          </div>
          <div class="unit-card-body">
            <div class="unit-title-row">
              <strong title="${escapeHtml(name)}">${escapeHtml(name)}</strong>
              <span>Lv ${formatNumber(unit.level || 1)}</span>
            </div>
            <div class="unit-badges">
              <span class="rarity-badge">${escapeHtml(rarity)}</span>
              ${unit.shiny === true ? '<span class="shiny-badge">✦ Shiny</span>' : ''}
            </div>
            <div class="unit-trait-row">${traitBadge(unit.trait) || '<span class="no-trait">No trait</span>'}</div>
            <div class="unit-extra${(unit.element || unit.archetype) ? '' : ' is-empty'}">${(unit.element || unit.archetype)
              ? escapeHtml([unit.element, unit.archetype].filter(Boolean).join(' · '))
              : '&nbsp;'}</div>
          </div>
        </article>`;
    }).join('');

    const remaining = Math.max(0, units.length - visibleUnits.length);
    elements.unitResultCount.textContent = `${formatNumber(visibleUnits.length)} / ${formatNumber(units.length)} units`;
    elements.unitLoadMore.hidden = remaining === 0;
    elements.unitLoadMore.textContent = remaining > 0 ? `Load more (${formatNumber(remaining)})` : '';
  ...
  function openInventoryModal(userId) {
    state.selectedInventoryUserId = userId; state.modalQuery = ''; state.modalSort = 'amount'; state.inventoryLimit = 48; state.inventoryVersion = '';
  ...
  function openUnitModal(userId) {
    state.selectedUnitUserId = userId; state.unitQuery = ''; state.unitRarity = 'all'; state.unitTrait = 'all'; state.unitLimit = 36; state.unitVersion = '';
  ...
  async function loadAvatars(players) {
    ...
    try {
      ...
      renderPlayers();
      const inventoryPlayer = selectedInventoryPlayer();
      const unitPlayer = selectedUnitPlayer();
      if (inventoryPlayer) fillPlayerModalHeader(inventoryPlayer, elements.modalName, elements.modalSubtitle, elements.modalAvatar);
      if (unitPlayer) fillPlayerModalHeader(unitPlayer, elements.unitName, elements.unitSubtitle, elements.unitAvatar);
      if (state.selectedDetailsUserId) renderDetailsModal();
    } catch { /* fallback */ }
  }

  async function loadAssetIcons() {
    const traitIds = Object.values(TRAIT_DEFINITIONS).map((definition) => definition.assetId);
    const assetIds = [...new Set([BAG_ASSET_ID, UNIT_BAG_ASSET_ID, PLAYER_EXP_ASSET_ID, WAVE_ASSET_ID, ...traitIds, ...ITEM_DEFINITIONS.map((definition) => definition.assetId)].filter(Boolean).map(Number))];
    try {
  ...
  async function loadPlayers() {
    ...
    try {
      ...
      elements.error.hidden = true; elements.lastRefresh.textContent = `Updated ${new Date().toLocaleTimeString('th-TH', { timeZone: THAI_TIME_ZONE, hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false })}`;
      renderMetrics(); renderRecent(); renderPlayers();

      const inventoryPlayer = selectedInventoryPlayer();
      const unitPlayer = selectedUnitPlayer();
      if (inventoryPlayer && String(inventoryPlayer.updatedAt || '') !== state.inventoryVersion) renderInventoryModal();
      if (unitPlayer && String(unitPlayer.updatedAt || '') !== state.unitVersion) renderUnitModal();
      if (state.selectedDetailsUserId) renderDetailsModal();
  ...
  const renderInventorySearch = debounce(() => {
    state.inventoryLimit = 48;
    renderInventoryModal();
  });
  const renderUnitSearch = debounce(() => {
    state.unitLimit = 36;
    renderUnitModal();
  });
  ...
  elements.unitRarity.addEventListener('change', (event) => {
    state.unitRarity = event.target.value;
    state.unitLimit = 36;
    renderUnitModal();
  });
  elements.unitTrait.addEventListener('change', (event) => {
    state.unitTrait = event.target.value;
    state.unitLimit = 36;
    renderUnitModal();
  });
  elements.unitLoadMore.addEventListener('click', () => {
    state.unitLimit += 36;
    renderUnitModal();
  });
