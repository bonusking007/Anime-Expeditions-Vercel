...
local function unwrapValue(Value)
    ...
    if type(Value) == "table" then
        for _, MethodName in ipairs({ "get", "Get", "peek", "Peek" }) do
            local Method = Value[MethodName]
            if type(Method) == "function" then
                local Success, Result = pcall(Method, Value)
                if Success and Result ~= nil then
                    return Result
                end
            end
...
local function readUnitField(Data, ...)
    ...
    for Index = 1, select("#", ...) do
        ...
        local Value = unwrapValue(Data[Key])
        if Value ~= nil then
            return Value
        end
    end
...
local function readTraitName(Value)
    Value = unwrapValue(Value)

    if type(Value) == "table" then
        Value = unwrapValue(Value.Name)
            or unwrapValue(Value.DisplayName)
            or unwrapValue(Value.Asset)
            or unwrapValue(Value.Id)
            or unwrapValue(Value.ID)
    end
...
local function getUnits(Replica)
    Replica = Replica or getReplica()
    local UnitData = Replica and Replica.Data and unwrapValue(Replica.Data.UnitData)

    -- บางเวอร์ชันของเกมห่อข้อมูล UnitData ไว้อีกชั้น
    if type(UnitData) == "table" then
        local Nested = unwrapValue(UnitData.Data)
            or unwrapValue(UnitData.Units)
            or unwrapValue(UnitData.Value)
    ...
    for UnitID, RawData in pairs(UnitData) do
        local Data = unwrapValue(RawData)

        -- รองรับรายการยูนิตที่เก็บข้อมูลจริงไว้ใน Data / Value / State
        if type(Data) == "table" then
            local NestedData = unwrapValue(Data.Data)
                or unwrapValue(Data.Value)
                or unwrapValue(Data.State)
        ...
        if type(Data) == "table" then
            local AssetValue = readUnitField(
                Data,
                "Asset",
                "Unit",
                "UnitAsset",
            ...
            local Asset = tostring(AssetValue or "")

            if Asset ~= "" then
                local Info = UnitsInfo and UnitsInfo[Asset]
                local LevelValue = readUnitField(Data, "Level", "Lvl", "UnitLevel")
                local Level = tonumber(LevelValue)
                    or tonumber(tostring(LevelValue or ""):match("%d+"))
                    or 1
                ...
                table.insert(Units, {
                    ...
                    trait = readTraitName(readUnitField(Data, "Trait", "TraitName", "Passive")),
                    shiny = readBoolean(readUnitField(Data, "Shiny", "IsShiny")),
                    equipped = readBoolean(readUnitField(Data, "Equipped", "IsEquipped")),
                    locked = readBoolean(readUnitField(Data, "Locked", "IsLocked")),
                    favorited = readBoolean(readUnitField(Data, "Favorited", "Favorite", "IsFavorited")),
...
local function getPlayerProgress()
    ...
    local CurrentXPNumberText = tostring(CurrentXP or "0"):gsub(",", "")
    local RequiredXPNumberText = tostring(RequiredXP or "0"):gsub(",", "")

    return {
        level = Level,
        currentXp = tonumber(CurrentXPNumberText) or 0,
        requiredXp = tonumber(RequiredXPNumberText) or 0,
        text = Text
    }
end
...
function sendSnapshot(force)
    ...
    local Units = {}
    local PlayerProgress = { level = 0, currentXp = 0, requiredXp = 0, text = "" }
    local CurrentWave, MaxWave = 0, 0

    local UnitSuccess, UnitResult = pcall(getUnits, Replica)
    if UnitSuccess and type(UnitResult) == "table" then
        Units = UnitResult
    else
        warn("อ่านข้อมูลยูนิตไม่สำเร็จ:", UnitResult)
    end

    local ProgressSuccess, ProgressResult = pcall(getPlayerProgress)
    if ProgressSuccess and type(ProgressResult) == "table" then
        PlayerProgress = ProgressResult
    else
        warn("อ่านเลเวลผู้เล่นไม่สำเร็จ:", ProgressResult)
    end

    local WaveSuccess, WaveValue, MaxWaveValue = pcall(getWaveProgress)
    if WaveSuccess then
        CurrentWave = tonumber(WaveValue) or 0
        MaxWave = tonumber(MaxWaveValue) or 0
    end
