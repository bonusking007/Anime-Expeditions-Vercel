        ...
        <div class="card-action">
          <div class="card-action-buttons">
            ...
            <button class="units-button" type="button" data-unit-user-id="${escapeHtml(player.userId)}">
              ${assetIconHtml(UNIT_BAG_ASSET_ID, 'Units', 'units-button-icon', '♟')}<span>View Units</span>
            </button>
  ...
  function renderInventoryModal() {
    ...
    elements.itemGrid.innerHTML = items.map((item) => `
      <article class="inventory-card">
        ...
        <div class="inventory-card-copy"><strong title="${escapeHtml(item.displayName)}">${escapeHtml(item.displayName)}</strong><span>${formatNumber(item.amount)}</span></div>
      </article>`).join('');
      ...
      return `
        <article class="unit-card rarity-${rarityClass(rarity)}">
          ...
          <div class="unit-card-body">
            ...
            ${(unit.element || unit.archetype) ? `<div class="unit-extra">${escapeHtml([unit.element, unit.archetype].filter(Boolean).join(' · '))}</div>` : ''}
          </div>
        </article>`;
    }).join('');
