...
local function getUnits(Replica)
    ...
    for UnitID, RawData in pairs(UnitData) do
        ...
        if type(Data) == "table" then
            local AssetValue = readUnitField(Data, "Asset", "Unit", "UnitAsset", "Name")
            local Asset = tostring(AssetValue or "")

            if Asset ~= "" then
                local Info = UnitsInfo and UnitsInfo[Asset]
                local LevelValue = readUnitField(Data, "Level", "Lvl")
                ...
                table.insert(Units, {
                    ...
                    trait = readTraitName(readUnitField(Data, "Trait", "TraitName")),
