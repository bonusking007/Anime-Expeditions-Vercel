(() => {
  ...
  const elements = {
    ...
    detailsItemTypes: $('details-item-types'),
  ...
  function getGemAmount(player) {
    ...
    for (const [name, amount] of Object.entries(player?.inventory || {})) {
      if (normalizeItemName(itemDisplayName(player, name)) === 'gem') total += Number(amount) || 0;
    }
    return total;
  }
    ...
    return `
      <article class="player-card card-corners">
        ...
        <div class="profile-block">
          ...
          <span class="profile-game">${escapeHtml(gameTitle(player))}</span>
          ${placeSubtitle(player) ? `<span class="profile-subgame">${escapeHtml(placeSubtitle(player))}</span>` : ''}
        ...
        <div class="card-action">
          <div class="card-action-buttons">
            ...
            <button class="details-button" type="button" data-details-user-id="${escapeHtml(player.userId)}">
              <span>◎</span>
              <span>Details</span>
            </button>
          </div>
  ...
  function renderPlayers() {
    ...
    elements.playerGrid.querySelectorAll('[data-inventory-user-id]').forEach((button) => {
      button.addEventListener('click', () => openInventoryModal(Number(button.dataset.inventoryUserId)));
    });
    elements.playerGrid.querySelectorAll('[data-details-user-id]').forEach((button) => {
      button.addEventListener('click', () => openDetailsModal(Number(button.dataset.detailsUserId)));
    });
  }
  ...
  function renderInventoryModal() {
    ...
    elements.modalName.textContent = maskPlayerName(player.displayName || player.username);
    elements.modalSubtitle.textContent = placeSubtitle(player)
      ? `${gameTitle(player)} · ${placeSubtitle(player)}`
      : gameTitle(player);
  ...
  function rewardIconFor(name) {
    ...
    if (definition.assetId) {
      return assetIconHtml(definition.assetId, definition.name, 'reward-icon', itemIconFallback(definition.name));
    }
    return `<span class="reward-icon icon-fallback" aria-hidden="true">${escapeHtml(itemIconFallback(name))}</span>`;
  }
  ...
  function renderDetailsModal() {
    ...
    elements.detailsServerLine.textContent = placeSubtitle(player)
      ? `${gameTitle(player)} · ${placeSubtitle(player)}`
      : gameTitle(player);
    elements.detailsDescription.textContent = playerDescription(player);
    elements.detailsItemTypes.textContent = formatNumber(player.itemTypes);
    elements.detailsTotalGems.textContent = formatNumber(getGemAmount(player));
    elements.detailsLastSync.textContent = relativeTime(player.ageSeconds);
    elements.detailsGame.textContent = gameTitle(player);
    ...
    const image = avatarUrl(player.userId);
    const fallback = escapeHtml((player.displayName || player.username).slice(0, 2).toUpperCase());
    elements.detailsAvatar.innerHTML = image
      ? `<img class="details-avatar-image" src="${escapeHtml(image)}" alt="Roblox avatar of ${escapeHtml(player.displayName || player.username)}" loading="lazy">`
      : `<div class="details-avatar-fallback">${fallback}</div>`;
    ...
    elements.detailsLogList.innerHTML = logs.length
      ? logs.map((log) => {
          const rewards = Array.isArray(log.rewards) ? log.rewards : [];
          const rewardsHtml = rewards.length
            ? rewards.map((reward) => `
                <span class="reward-chip">
                  ${rewardIconFor(reward.name)}
                  <span>${escapeHtml(reward.name)} x${formatNumber(reward.amount)}</span>
                </span>
              `).join('')
            : '<span class="detail-chip muted">No rewards found</span>';

          const isVictory = String(log.result || '').toLowerCase().includes('victory');
          const stageText = log.stageName || placeSubtitle(player) || gameTitle(player);
          const description = log.description || '';
          const dateValue = Number(log.finishedAt)
            ? new Date(Number(log.finishedAt) > 9_999_999_999 ? Number(log.finishedAt) : Number(log.finishedAt) * 1000)
            : null;
          ...
          return `
            <article class="result-entry ${isVictory ? 'victory' : 'defeat'}">
              <div class="result-entry-top">
                <div>
                  <span class="result-state">${escapeHtml(log.result || 'Victory')}</span>
                  <h3>${escapeHtml(stageText)}</h3>
                </div>
                <div class="result-entry-side">
                  ${log.clearTime ? `<strong>${escapeHtml(log.clearTime)}</strong>` : ''}
                  <span>${escapeHtml(dateValue ? formatDateTime(dateValue) : '--')}</span>
                </div>
              </div>
      ...
      : '<div class="details-empty">No run logs yet. Finish a game and let the tracker sync again.</div>';
  }

  function openInventoryModal(userId) {
    state.selectedInventoryUserId = userId;
    state.modalQuery = '';
  ...
  function closeInventoryModal() {
    state.selectedInventoryUserId = null;
    elements.modal.hidden = true;
    if (elements.detailsModal.hidden) document.body.style.overflow = '';
  }

  function openDetailsModal(userId) {
    state.selectedDetailsUserId = userId;
  ...
  function closeDetailsModal() {
    state.selectedDetailsUserId = null;
    elements.detailsModal.hidden = true;
    if (elements.modal.hidden) document.body.style.overflow = '';
  }
  ...
  async function loadAvatars(players) {
    ...
    try {
      ...
      Object.entries(data.avatars).forEach(([rawId, imageUrl]) => {
        ...
        if (Number.isFinite(userId) && typeof imageUrl === 'string' && imageUrl) state.avatars.set(userId, imageUrl);
      });

      renderPlayers();
      if (state.selectedInventoryUserId) renderInventoryModal();
  ...
  async function loadAssetIcons() {
    const assetIds = [...new Set([BAG_ASSET_ID, ...ITEM_DEFINITIONS.map((definition) => definition.assetId)].filter(Boolean).map(Number))];
    ...
    try {
      for (let index = 0; index < assetIds.length; index += 100) {
        ...
        Object.entries(data.icons).forEach(([rawId, imageUrl]) => {
          ...
          if (Number.isFinite(assetId) && typeof imageUrl === 'string' && imageUrl) state.assetIcons.set(assetId, imageUrl);
        });
      }
  ...
  elements.modal.addEventListener('mousedown', (event) => {
    if (event.target === elements.modal) closeInventoryModal();
  });
  ...
  elements.detailsModal.addEventListener('mousedown', (event) => {
    if (event.target === elements.detailsModal) closeDetailsModal();
  });
  document.addEventListener('keydown', (event) => {
    if (event.key !== 'Escape') return;
    if (!elements.detailsModal.hidden) closeDetailsModal();
    else if (!elements.modal.hidden) closeInventoryModal();
  });
